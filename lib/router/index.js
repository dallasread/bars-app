...
Router.definePrototype({
    ...
    transitionTo: function transitionTo(route) {
        ...
        async.eachSeries(routesToUnload, function iterator(r, next) {
            ...
            async.eachSeries(route.ancestors, function performBeforeLoads(r, next) {
    ...
    update: function update(data) {
        ...
        if (_.currentRoute) {
            _.currentRoute.update(data || _._data);

            for (var i = _.currentRoute.ancestors.length - 1; i >= 0; i--) {
                route = _.currentRoute.ancestors[i];
                route.update(data || route._data);
            }
        }
